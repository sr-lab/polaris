...
Section recurrence_counter_sec.
  ...
  Definition size (x: X) := INR x.
  Hypothesis T_non_neg: ∀ x n, (T x) n ≥ 0.
  Hypothesis hrange_dec: ∀ x n, size x > 1 → size ((h x) n) ≤ size x - 1.
  ...
  Proof.
    ...
    - apply mnondec.
    - apply d_non_neg.
    - eapply decreasing_rec.decreasing_rec.hinf_0 with (msr := size) (msr_cutoff := d);
        rewrite /d; eauto using hrange_dec, hrange1, hrange2.
      nra. intros. etransitivity; eauto.
